...
fn isAlpha(c: u8) bool {
    return switch (c) {
        inline 'a'...'z', 'A'...'Z', '_' => true,
        else => false,
    };
}

pub fn next(self: *Scanner) ?t.Token {
    var nextTok = self.scanToken() catch return null;
...
fn number(self: *Scanner) t.Token {
    ...
    if (self.peek() == '.' and isDigit(self.peekNext())) {
        _ = self.advance();
        while (isDigit(self.peek())) : (_ = self.advance()) {}
        return self.makeToken(t.TokenType.number);
    }
    return self.makeToken(t.TokenType.number);
}
...
test "numbers" {
    const source = "3 3.14 3.integer";
    var zero: usize = 0;
    var scanner = Scanner{ .source = source[zero..source.len] };
    const numTok = try scanner.scanToken();
    try std.testing.expectEqual(t.TokenType.number, numTok.type);

    const floatTok = try scanner.scanToken();
    try std.testing.expectEqual(t.TokenType.number, floatTok.type);
    const dotTok = try scanner.scanToken();
    try std.testing.expectEqual(t.TokenType.@"error", dotTok.type);
...
test "non keywords" {
    ...
    while (tok != null) : (tok = scanner.next()) {
...
test "mixed" {
    ...
    var tok = scanner.next();
    try std.testing.expectEqual(tok.?.type, t.TokenType.number);
